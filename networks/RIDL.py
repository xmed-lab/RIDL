...
class RIDL(nn.Module):
    def __init__(self, training, in_channel_0 = 1, in_channel_1 = 1, dropout = 0.3, fconnect = False, fc_layer_num = 2, glb_ft_num = 2):
        super().__init__()

        self.training = training
        self.out_channel = in_channel_0
        self.in_channel = in_channel_0
        ...
        self.down_conv1 = nn.Sequential(
            nn.Conv3d(16 + in_channel_1, 32, 2, 2),
            nn.PReLU(32)
        )
        ...
        self.ftfuscam = ChannelGate(gate_channels = 16 + in_channel_1, reduction_ratio = 2)


    def forward(self, inputs_0, inputs_1, glb_shp_ft):
        if self.in_channel == 1:
            long_range1 = self.encoder_stage1(inputs_0) + inputs_0
        else:
            long_range1 = self.encoder_stage1(inputs_0)


        # print(long_range1.shape)
        layer_1_exp = long_range1
        long_range1 = torch.cat((long_range1, inputs_1), dim = 1)
        long_range1 = self.ftfuscam(long_range1)
        ...
        if self.training is True:
            return output1, output2, output3, output4, output_cls, outputs_encoded, spat_scale, layer_1_exp
        else:
